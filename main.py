...
# Filename: main.py
# Program for presentation of NY Subway System
# Code derived from Filbert88 on GithubNO
...
# get user input for displaying list of stations
def display_stations_prompt():
    global inputs 
    inputs = input("\nDo you want to see the list of the stations (YES/NO): ")
    if inputs.lower() == "yes":
        display_stations(nodes)
    elif inputs.lower() != "no":
        print("Invalid Input. Please Try again.")
        display_stations_prompt()
    return inputs

# get source station from user input
def get_source():
    global source
...
# get destination station from user input
def get_destination():
    global destination
...
# get user input for viewing visualization of graph
def display_graph_prompt():
    global display
...
print("\nWelcome to the NY Subway System Route Finder!")
